...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: getTranslation(locale, "teachers.title"),
    description: getTranslation(locale, "teachers.description"),
  };
}
    ...
    <>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <h2 className="font-heading text-2xl font-semibold text-ink mb-6">
          {t("teachers.credentialsHeading")}
        </h2>
        <div className="space-y-3 max-w-xl">
          <div className="flex items-center gap-3 font-body text-sm text-ink/70">
            <span className="w-2 h-2 rounded-full bg-gold shrink-0" />
            {t("teachers.credentialScore")}
          </div>
          <div className="flex items-center gap-3 font-body text-sm text-ink/70">
            <span className="w-2 h-2 rounded-full bg-gold shrink-0" />
            {t("teachers.credentialCertificate")}
          </div>
          <div className="flex items-center gap-3 font-body text-sm text-ink/70">
            <span className="w-2 h-2 rounded-full bg-gold shrink-0" />
            {t("teachers.credentialExperience")}
          </div>
        </div>
      </section>
